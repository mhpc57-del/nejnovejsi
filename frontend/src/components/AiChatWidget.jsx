          ...
          <div className="bg-gray-900 px-4 py-3 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs">Online podpora</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
            {messages.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 text-xs">Zeptejte se mě na cokoliv ohledně platformy.</p>
                <div className="mt-4 space-y-1.5">
                  {['Jak se zaregistruji?', 'Jaké jsou tarify?', 'Jak funguje poptávka?'].map((q) => (
                    <button
                      key={q}
                      onClick={() => sendQuickQuestion(q)}
                      className="block w-full text-left px-3 py-2 bg-white rounded-lg text-xs text-gray-600 hover:bg-orange-50 hover:text-orange-600 border border-gray-100 transition-colors"
              ...
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex items-end gap-1.5 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
                    ...
                    {msg.role === 'user'
                      ? <User weight="bold" className="w-3.5 h-3.5 text-gray-600" />
          ...
          <div className="p-3 border-t border-gray-100 bg-white flex-shrink-0">
            <div className="flex gap-2">
              <input
                ...
                disabled={loading}
                className="flex-1 px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 disabled:opacity-50"
