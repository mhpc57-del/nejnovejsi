...
const AdminDashboard = () => {
  ...
  const getStatusBadge = (status) => {
    const map = {
      ...
      completed: { style: 'bg-zinc-100 dark:bg-zinc-800 text-zinc-700', label: 'Dokončeno' },
      cancelled: { style: 'bg-red-100 text-red-700', label: 'Zrušeno' },
    };
    const s = map[status] || { style: 'bg-zinc-100 dark:bg-zinc-800 text-zinc-700', label: status };
  ...
  const getRoleBadge = (role) => {
    ...
    const r = map[role] || { style: 'bg-zinc-100 dark:bg-zinc-800', label: role };
    ...
    <div className="p-6">
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Dokončených', value: stats?.completed_demands || 0, icon: Check, color: 'bg-zinc-50 dark:bg-zinc-800/500' },
          { label: 'Čekající kategorie', value: stats?.pending_suggestions || 0, icon: Tag, color: 'bg-yellow-500' },
        ].map((stat, i) => (
          <div key={i} className="bg-zinc-50 dark:bg-zinc-800/50 rounded-xl p-5" data-testid={`stat-${i}`}>
      ...
      <h3 className="font-semibold text-zinc-900 dark:text-white mb-4">Poslední zakázky</h3>
      <div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-xl overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-zinc-200">
              <th className="text-left p-4 text-sm font-medium text-zinc-500">Název</th>
              <th className="text-left p-4 text-sm font-medium text-zinc-500">Kategorie</th>
              <th className="text-left p-4 text-sm font-medium text-zinc-500">Stav</th>
              <th className="text-left p-4 text-sm font-medium text-zinc-500">Datum</th>
              <th className="text-left p-4 text-sm font-medium text-zinc-500"></th>
            </tr>
          </thead>
          <tbody>
            {demands.slice(0, 5).map((demand) => (
              <tr key={demand.id} className="border-b border-zinc-200/80 dark:border-zinc-800 last:border-0">
                <td className="p-4 text-sm text-zinc-900 dark:text-white">{demand.title}</td>
                <td className="p-4 text-sm text-zinc-500">{demand.category}</td>
                <td className="p-4">{getStatusBadge(demand.status)}</td>
                <td className="p-4 text-sm text-zinc-500">{new Date(demand.created_at).toLocaleDateString('cs-CZ')}</td>
                <td className="p-4">
    ...
    <div className="p-4">
      <div className="mb-4 flex items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Hledat uživatele..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 border border-zinc-200 rounded-xl text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500" data-testid="search-users" />
        </div>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-zinc-200">
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">E-mail</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Jméno / Firma</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Role</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Stav</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Důvěra</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Registrace</th>
              <th className="text-right p-3 text-xs font-medium text-zinc-500 uppercase">Akce</th>
            </tr>
          </thead>
          <tbody>
            {filteredUsers.map((u) => (
              <tr key={u.id} className={`border-b border-zinc-200/80 dark:border-zinc-800 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 ${u.is_blocked ? 'bg-red-50/50' : ''}`} data-testid={`user-row-${u.id}`}>
                <td className="p-3 text-sm text-zinc-900 dark:text-white">{u.email}</td>
                <td className="p-3 text-sm text-zinc-600">{u.company_name || `${u.first_name || ''} ${u.last_name || ''}`.trim() || '-'}</td>
                ...
                <td className="p-3">
                  {u.is_blocked ? (
                    ...
                    <span className="px-2 py-1 rounded-full text-xs font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-600">Deaktivován</span>
                  ) : u.is_verified ? (
                ...
                <td className="p-3 text-xs text-zinc-500">{new Date(u.created_at).toLocaleDateString('cs-CZ')}</td>
                <td className="p-3">
                  {u.role !== 'admin' && (
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => { 
                        ...
                        className="p-1.5 text-zinc-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Upravit" data-testid={`edit-user-${u.id}`}>
                        <PencilSimple className="w-4 h-4" />
                      </button>
                      <button onClick={() => { setModal({ type: 'messageUser', data: u }); setModalInput({ subject: '', message: '' }); }}
                        className="p-1.5 text-zinc-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors" title="Poslat zprávu" data-testid={`message-user-${u.id}`}>
                        <Envelope className="w-4 h-4" />
                      </button>
                      {!u.is_verified && (
                        <button onClick={() => handleAction('verificationReminder', u.id)}
                          className="p-1.5 text-zinc-400 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-colors" title="Připomenout ověření" data-testid={`verify-remind-${u.id}`}>
                          <Warning className="w-4 h-4" />
                        </button>
                      )}
                      {u.is_deactivated && (
                        <button onClick={() => handleAction('reactivate', u.id)}
                          className="p-1.5 text-zinc-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Reaktivovat účet" data-testid={`reactivate-user-${u.id}`}>
                          <ArrowClockwise className="w-4 h-4" />
                        </button>
                      )}
                      {u.is_blocked ? (
                        <button onClick={() => handleAction('unblock', u.id)}
                          className="p-1.5 text-zinc-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Odblokovat" data-testid={`unblock-user-${u.id}`}>
                          <LockOpen className="w-4 h-4" />
                        </button>
                      ) : (
                        <button onClick={() => setModal({ type: 'blockUser', data: u })}
                          className="p-1.5 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Zablokovat" data-testid={`block-user-${u.id}`}>
                          <Lock className="w-4 h-4" />
                        </button>
                      )}
                      <button onClick={() => setModal({ type: 'deleteUser', data: u })}
                        className="p-1.5 text-zinc-400 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors" title="Smazat uživatele" data-testid={`delete-user-${u.id}`}>
    ...
    <div className="p-4">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Hledat zakázky..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 border border-zinc-200 rounded-xl text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500" data-testid="search-demands" />
        </div>
        <select value={demandFilter} onChange={e => setDemandFilter(e.target.value)}
          className="px-3 py-2.5 border border-zinc-200 rounded-xl text-sm" data-testid="filter-demands">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-zinc-200">
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Název</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Kategorie</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Zákazník</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Dodavatel</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Stav</th>
              <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Datum</th>
              <th className="text-right p-3 text-xs font-medium text-zinc-500 uppercase">Akce</th>
            </tr>
          </thead>
          <tbody>
            {filteredDemands.map((d) => (
              <tr key={d.id} className="border-b border-zinc-200/80 dark:border-zinc-800 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/50" data-testid={`demand-row-${d.id}`}>
                <td className="p-3 text-sm text-zinc-900 dark:text-white max-w-[200px] truncate">{d.title}</td>
                <td className="p-3 text-sm text-zinc-500">{d.category}</td>
                <td className="p-3 text-sm text-zinc-500">{d.customer_name}</td>
                <td className="p-3 text-sm text-zinc-500">{d.assigned_supplier_name || '-'}</td>
                <td className="p-3">{getStatusBadge(d.status)}</td>
                <td className="p-3 text-xs text-zinc-500">{new Date(d.created_at).toLocaleDateString('cs-CZ')}</td>
                <td className="p-3">
                  <div className="flex items-center gap-1 justify-end">
                    <Link to={`/zakazka/${d.id}`} className="p-1.5 text-zinc-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors" title="Detail" data-testid={`view-demand-${d.id}`}>
  ...
  const renderCategories = () => (
    <div className="p-6">
      <div className="mb-6">
        <h3 className="font-semibold text-zinc-900 dark:text-white mb-1">Navržené kategorie</h3>
        <p className="text-sm text-zinc-500">Dodavatelé mohou navrhovat nové kategorie služeb. Zde je schválíte nebo zamítnete.</p>
      </div>
          ...
          <div className="space-y-3">
            {pendingSuggestions.map(s => (
              <div key={s.id} className="flex items-center justify-between bg-orange-50 border border-orange-200 rounded-xl p-4" data-testid={`suggestion-${s.id}`}>
                <div>
                  <p className="font-medium text-zinc-900 dark:text-white">{s.category_name}</p>
                  <p className="text-sm text-zinc-500">Navrhl: {s.suggested_by_name} | {new Date(s.created_at).toLocaleDateString('cs-CZ')}</p>
                </div>
      ...
      {suggestions.filter(s => s.status !== 'pending').length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-zinc-500 mb-3">Historie</h4>
          <div className="space-y-2">
            {suggestions.filter(s => s.status !== 'pending').map(s => (
              <div key={s.id} className={`flex items-center justify-between rounded-xl p-4 ${s.status === 'approved' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                <div>
                  <p className="font-medium text-zinc-900 dark:text-white">{s.category_name}</p>
                  <p className="text-sm text-zinc-500">Navrhl: {s.suggested_by_name}</p>
                </div>
      ...
      {suggestions.length === 0 && (
        <p className="text-center text-zinc-400 py-10">Zatím žádné návrhy kategorií.</p>
      )}
    </div>
  );
  ...
  const renderInvoices = () => (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-bold text-zinc-900 dark:text-white">Faktury</h2>
          <p className="text-sm text-zinc-500">Prehled vsech faktur platformy</p>
        </div>
        <div className="flex items-center gap-3">
          <input type="month" value={invoiceMonth} onChange={(e) => setInvoiceMonth(e.target.value)}
            className="px-3 py-2 border border-zinc-200 rounded-lg text-sm" data-testid="invoice-month-filter" />
      ...
      {invoicesLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-zinc-500 text-sm">Klikněte "Zobrazit" pro načtení faktur za vybraný měsíc</p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full" data-testid="admin-invoices-table">
            <thead>
              <tr className="border-b border-zinc-200/80 dark:border-zinc-800">
                <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Číslo</th>
                <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Zákazník</th>
                <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Datum</th>
                <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Plán</th>
                <th className="text-right p-3 text-xs font-medium text-zinc-500 uppercase">Částka</th>
                <th className="text-left p-3 text-xs font-medium text-zinc-500 uppercase">Stav</th>
                <th className="text-right p-3 text-xs font-medium text-zinc-500 uppercase">Akce</th>
              </tr>
            </thead>
            <tbody>
              {adminInvoices.map((inv) => (
                <tr key={inv.id} className="border-b border-gray-50 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/50" data-testid={`admin-invoice-${inv.id}`}>
                  <td className="p-3 text-sm font-medium text-zinc-900 dark:text-white">{inv.invoice_number}</td>
                  <td className="p-3 text-sm text-zinc-600">{inv.customer?.name || inv.user_email}</td>
                  <td className="p-3 text-sm text-zinc-500">{new Date(inv.issue_date).toLocaleDateString('cs-CZ')}</td>
                  <td className="p-3 text-sm text-zinc-500">{inv.plan_name || '-'}</td>
                  <td className="p-3 text-sm font-semibold text-zinc-900 dark:text-white text-right">{inv.total?.toLocaleString('cs-CZ')} Kc</td>
                  <td className="p-3">
  ...
  const renderModal = () => {
    ...
    const modalContent = {
      blockUser: {
        ...
        body: <p className="text-zinc-600 mb-4">Opravdu chcete zablokovat tohoto uživatele? Nebude se moci přihlásit a obdrží emailové oznámení.</p>,
      ...
      deleteUser: {
        ...
        body: (
          <div className="space-y-3">
            ...
            <p className="text-zinc-600">Budou smazány veškeré údaje uživatele včetně jeho poptávek, zpráv, recenzí a faktur. Uživatel se poté může znovu zaregistrovat pod stejným emailem.</p>
          </div>
        ),
      ...
      rejectCategory: {
        ...
        body: (
          <div className="space-y-3">
            <p className="text-zinc-600">Navrhovateli bude odeslán email s oznámením o zamítnutí.</p>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Důvod zamítnutí (volitelné)</label>
              <textarea value={modalInput.reason || ''} onChange={e => setModalInput(prev => ({ ...prev, reason: e.target.value }))} rows={3}
                className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" placeholder="Např.: Kategorie je příliš úzká, spadá pod existující kategorii..." data-testid="modal-reject-reason" />
            </div>
          </div>
        ),
      ...
      editUser: {
        ...
        body: (
          <div className="space-y-3 max-h-[55vh] overflow-y-auto pr-2">
            <p className="text-xs text-zinc-400 uppercase font-medium tracking-wider mb-1">Přihlašovací údaje</p>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">E-mail</label>
              <input type="email" value={modalInput.email || ''} onChange={e => setModalInput(prev => ({ ...prev, email: e.target.value }))}
                className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" data-testid="modal-email" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 mb-1">Typ účtu</label>
                <select value={modalInput.account_type || ''} onChange={e => setModalInput(prev => ({ ...prev, account_type: e.target.value }))}
                  className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" data-testid="modal-account-type">
                  <option value="">-</option>
            ...
            <div className="border-t border-zinc-200/80 dark:border-zinc-800 pt-3 mt-3">
              <p className="text-xs text-zinc-400 uppercase font-medium tracking-wider mb-2">Osobní údaje</p>
            </div>
            {[
              ...
              <div key={f.key}>
                ...
                <div className={f.hasAres ? 'flex gap-2' : ''}>
                  <input type="text" value={modalInput[f.key] || ''} onChange={e => setModalInput(prev => ({ ...prev, [f.key]: e.target.value }))}
                    className={`${f.hasAres ? 'flex-1' : 'w-full'} px-3 py-2 border border-zinc-200 rounded-lg text-sm`} data-testid={`modal-${f.key}`} />
                  {f.hasAres && (
            ...
            <div className="border-t border-zinc-200/80 dark:border-zinc-800 pt-3 mt-3">
              <p className="text-xs text-zinc-400 uppercase font-medium tracking-wider mb-2">Adresy</p>
            </div>
            {[
              ...
              <div key={f.key}>
                <label className="block text-sm font-medium text-zinc-700 mb-1">{f.label}</label>
                <input type="text" value={modalInput[f.key] || ''} onChange={e => setModalInput(prev => ({ ...prev, [f.key]: e.target.value }))}
                  className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" data-testid={`modal-${f.key}`} />
              </div>
            ))}
            <div className="border-t border-zinc-200/80 dark:border-zinc-800 pt-3 mt-3">
              <p className="text-xs text-zinc-400 uppercase font-medium tracking-wider mb-2">Popis</p>
            </div>
            <div>
              ...
              <textarea value={modalInput.bio || ''} onChange={e => setModalInput(prev => ({ ...prev, bio: e.target.value }))} rows={3}
                className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" data-testid="modal-bio" />
            </div>
          </div>
        ),
      ...
      messageUser: {
        ...
        body: (
          <div className="space-y-3">
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Předmět</label>
              <input type="text" value={modalInput.subject || ''} onChange={e => setModalInput(prev => ({ ...prev, subject: e.target.value }))}
                className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" placeholder="Např.: Informace o vašem účtu" data-testid="modal-subject" />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Zpráva</label>
              <textarea value={modalInput.message || ''} onChange={e => setModalInput(prev => ({ ...prev, message: e.target.value }))} rows={5}
                className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" placeholder="Text emailu..." data-testid="modal-message" />
            </div>
          </div>
        ),
      ...
      cancelDemand: {
        ...
        body: (
          <div className="space-y-3">
            <p className="text-zinc-600">Zákazník obdrží emailové oznámení s důvodem zrušení.</p>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Důvod zrušení</label>
              <textarea value={modalInput.reason || ''} onChange={e => setModalInput(prev => ({ ...prev, reason: e.target.value }))} rows={3}
                className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" placeholder="Popište důvod..." data-testid="modal-reason" />
            </div>
          </div>
        ),
      ...
      notifyDemand: {
        ...
        body: (
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Doplňující zpráva (volitelné)</label>
              <textarea value={modalInput.message || ''} onChange={e => setModalInput(prev => ({ ...prev, message: e.target.value }))} rows={3}
                className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" placeholder="Vaše zpráva zákazníkovi..." data-testid="modal-notify-message" />
            </div>
          </div>
        ),
    ...
    return (
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => { setModal(null); setModalInput({}); }} data-testid="admin-modal-overlay">
        <div className="bg-white dark:bg-zinc-900 rounded-xl max-w-lg w-full max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()} data-testid="admin-modal">
          <div className="p-6 border-b border-zinc-200/80 dark:border-zinc-800">
            <h3 className="text-lg font-semibold text-zinc-900 dark:text-white">{mc.title}</h3>
          </div>
          <div className="p-6">
            {mc.body}
          </div>
          <div className="p-6 border-t border-zinc-200/80 dark:border-zinc-800 flex gap-3 justify-end">
            <button onClick={() => { setModal(null); setModalInput({}); }}
              className="px-4 py-2.5 text-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 rounded-xl text-sm font-medium" data-testid="modal-cancel">
              Zrušit
            </button>
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-zinc-950">
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-zinc-200/80 dark:border-zinc-800 p-6 hidden lg:block">
        ...
        <nav className="space-y-2">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSearchQuery(''); }}
              className={`flex items-center gap-3 px-4 py-3 w-full rounded-xl transition-colors ${activeTab === tab.id ? 'bg-orange-500 text-white' : 'text-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`} data-testid={`nav-${tab.id}`}>
        ...
        <div className="absolute bottom-6 left-6 right-6">
          <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 w-full text-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 rounded-xl transition-colors" data-testid="logout-btn">
      ...
      <main className="lg:ml-64 min-h-screen">
        <header className="bg-white/80 dark:bg-zinc-950/70 backdrop-blur-xl border-b border-zinc-200/60 dark:border-zinc-800/60 px-6 py-4 sticky top-0 z-40">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500">Správa platformy CraftBolt</p>
            ...
            <div className="flex gap-1 lg:hidden">
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSearchQuery(''); }}
                  className={`p-2 rounded-lg ${activeTab === tab.id ? 'bg-orange-500 text-white' : 'text-zinc-400'}`}>
        ...
        <div className="p-6">
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200/80 dark:border-zinc-800">
            {renderContent()}
          </div>
        </div>
      </main>
...
const DemandActionsMenu = ({ demand, onAction }) => {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="p-1.5 text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 dark:bg-zinc-800 rounded-lg transition-colors" data-testid={`demand-actions-${demand.id}`}>
        <CaretDown className="w-4 h-4" />
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-30" onClick={() => setOpen(false)} />
          <div className="absolute right-0 top-full mt-1 bg-white border border-zinc-200 rounded-xl shadow-lg z-40 py-1 w-56" data-testid={`demand-menu-${demand.id}`}>
            <button onClick={() => { onAction('wrong_category'); setOpen(false); }}
              className="w-full text-left px-4 py-2.5 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800/50 flex items-center gap-2 text-zinc-700" data-testid={`notify-wrong-cat-${demand.id}`}>
              <Tag className="w-4 h-4 text-orange-500" /> Špatná kategorie
            </button>
            <button onClick={() => { onAction('improve_description'); setOpen(false); }}
              className="w-full text-left px-4 py-2.5 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800/50 flex items-center gap-2 text-zinc-700" data-testid={`notify-improve-${demand.id}`}>
              <PencilSimple className="w-4 h-4 text-blue-500" /> Vylepšit popis
            </button>
            <button onClick={() => { onAction('vulgar_language'); setOpen(false); }}
              className="w-full text-left px-4 py-2.5 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800/50 flex items-center gap-2 text-red-600" data-testid={`notify-vulgar-${demand.id}`}>
              <Warning className="w-4 h-4" /> Nevhodná slova / vulgarita
            </button>
            <button onClick={() => { onAction('custom'); setOpen(false); }}
              className="w-full text-left px-4 py-2.5 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800/50 flex items-center gap-2 text-zinc-700" data-testid={`notify-custom-${demand.id}`}>
              <ChatCircle className="w-4 h-4 text-zinc-500" /> Vlastní oznámení
            </button>
            <div className="border-t border-zinc-200/80 dark:border-zinc-800 mt-1 pt-1">
