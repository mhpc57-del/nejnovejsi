        ...
        <div className="space-y-6" data-testid="supplier-profile-content">
          ...
          <div className="bg-white dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl p-6">
            <div className="flex items-center gap-5">
              <div className="w-16 h-16 rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center text-2xl font-bold text-orange-500">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  <span className="text-sm text-zinc-500 dark:text-zinc-400">Dodavatel</span>
                  ...
                  {!user?.subscription_active && (
                    <span className="px-2 py-0.5 bg-red-100 text-red-600 text-xs font-semibold rounded-full">Neaktivní přístup</span>
                  )}
                </div>
              </div>
            </div>
          ...
          {/* Paywall */}
          {!user?.subscription_active && (
            <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-800 rounded-xl p-5" data-testid="paywall-banner">
          ...
          <div className="bg-white dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={f.key}>
                  <label className="block text-sm text-zinc-500 dark:text-zinc-400 mb-1">{f.label}</label>
                  {editingProfile ? (
                    <input value={profileForm[f.key] || ''} onChange={e => setProfileForm(p => ({...p, [f.key]: e.target.value}))}
                      className="w-full px-3 py-2.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-zinc-900 dark:text-white text-sm" />
                  ) : (
                    <p className="px-3 py-2.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-zinc-900 dark:text-white text-sm">{profile?.[f.key] || '—'}</p>
                  )}
                </div>
              ))}
            </div>
            {profile?.company_name && (
              <div className="mt-4">
                <label className="block text-sm text-zinc-500 dark:text-zinc-400 mb-1">Firma</label>
                {editingProfile ? (
                  <input value={profileForm.company_name || ''} onChange={e => setProfileForm(p => ({...p, company_name: e.target.value}))}
                    className="w-full px-3 py-2.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-zinc-900 dark:text-white text-sm" />
                ) : (
                  <p className="px-3 py-2.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-zinc-900 dark:text-white text-sm">{profile?.company_name}</p>
                )}
              </div>
            )}
            <div className="mt-4">
            ...
            <div className="mt-4 flex items-center gap-3">
              <button onClick={() => setProfileForm(p => ({...p, sms_notifications: !p.sms_notifications}))}
            ...
            {profile?.categories?.length > 0 && (
              <div className="mt-5">
                <label className="block text-sm text-zinc-500 dark:text-zinc-400 mb-2">Kategorie služeb</label>
